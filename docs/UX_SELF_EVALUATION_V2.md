...
The following items were found by cross-referencing the v2 plan's specific sub-items against completed code and the v3 plan. These are details *within the v2 plan itself* that were neither implemented to spec nor tracked for future work:

33. **Status bar P/L missing `mc-glow-green`** — The v2 plan (Phase 1, line 44) specified the earnings stat should use `mc-text-green` with `mc-glow-green` when positive. GameTracking's Running Tally uses `mc-glow-green` correctly, but `GameStatusBar.tsx` only uses `mc-text-green` — no glow. A small visual inconsistency between the same data point shown in two places.

34. **Header tab font size 11px vs v2 spec's 13px** — The v2 plan (Phase 2, line 107) specified `13px` for tab font size. The actual implementation uses `11px`. The v3 plan Phase 1.6 proposes shrinking further to `10px` at narrow breakpoints — moving further from the spec rather than correcting to it. The original 13px target was never reached.

35. **Podium uses medal icons, not avatar/initial circles** — The v2 plan (Phase 9, line 504) said each podium block should show an "Avatar/initial circle." The implementation shows medal icons (Crown, Medal) inside colored circles. The code even has a comment saying "Avatar + name" but the actual content is a medal icon, not a user initial. Not catastrophic, but the plan wanted user identity (initial letter) while the implementation shows rank identity (medal).

36. **QR code "Download QR" button** — The v2 plan (Phase 5.2, line 295) said "The QR code should be downloadable (wrap in a canvas and provide a 'Download QR' button)." The v3 plan Phase 7.1 adds the QR display using `qrcode.react` but doesn't include a download button or canvas wrapping. The display-only version covers the primary use case (scan with phone) but the download feature was explicitly specified.

37. **"Last Payout" stat on splash ribbon** — The v2 plan (Phase 10, line 553-556) specified three stats: "Pot: 847 ICP | 12 Players | Last Payout: 2.4 ICP". The implementation and the v3 plan both use different stats (Pot + Players + "Live on ICP"). "Last Payout" creates specific social proof — someone recently got paid — which is more compelling than a generic "Live on ICP" label.

38. **Celebration timer 4 seconds vs v2 spec's 3 seconds** — The v2 plan (Phase 11, line 607) said "After 3 seconds, auto-navigate." The implementation uses `4000`. The v3 plan Phase 1.3 fixes the refetch, adds a proceed button, and replaces the empty callback — but never corrects the timer duration.

39. **House Ledger accordion initial state** — The v2 plan (Phase 7.1, line 402) said "Default: all collapsed (the first one optionally expanded)." The implementation starts all sections collapsed. The v2 self-evaluation flagged this as a "missed polish opportunity" but the v3 plan never addresses the initial state. The common progressive disclosure pattern is first-section-open so users see content immediately rather than facing a wall of closed headers.

The following items were found by auditing the UX Design Philosophy doc's rules against the current implementation, and by re-reading the v2 self-evaluation prose for specific criticisms that were never promoted to tracked items:

40. **Header tabs violate `--space-sm` (8px) minimum spacing rule** — The Design Philosophy doc says "Never less than `--space-sm` (8px) between interactive elements." The `.mc-header-tabs` container uses `gap: 2px` between separately clickable tab buttons. That's a 75% violation of the Design Philosophy's own minimum. The v3 plan addresses overflow (C-6, Phase 1.6) and font size (B-34, Phase 17.2) but never audits the inter-element spacing against the documented rule. The mobile bottom tabs also use `gap: 2px` within each tab, but that's within a single interactive element — the header tabs are separate clickable items.

41. **Perpetual `mc-tagline-bob` animation violates Design Philosophy animation rules** — The Design Philosophy doc says animations must be "Purposeful only" and explicitly lists "perpetual bounce on decoration" as a "Bad" example. The tagline ("It's a Ponzi!") has `animation: mc-tagline-bob 3s ease-in-out infinite` — a perpetual bobbing animation on a static decorative text element. This is exactly the anti-pattern the Design Philosophy doc identifies. The v3 plan adds a typewriter effect to the tagline (B-1, Phase 8.2) but doesn't address removing the perpetual bob. Both can't coexist — the typewriter animates the text appearing, the bob animates it bouncing forever after.

42. **Color token inconsistency — Tailwind classes used instead of design system tokens** — The v2 self-eval Phase 3 prose noted "Colors use `bg-purple-500` / `bg-green-500` instead of the mc-text-* tokens" but this was never promoted to a tracked item. GameTracking.tsx uses `bg-purple-500` for compounding position bars, `bg-green-500` for simple position bars, and `bg-purple-500/20` / `bg-green-500/20` for toll badges and plan type badges. These are Tailwind utility classes, not the `mc-text-purple` / `mc-text-green` design system tokens. The v3 plan has a typography audit (B-9, Phase 12.1) but no color token audit. Typography and color are different concerns — auditing one doesn't cover the other.

43. **Header content density is a design problem, not just an overflow problem** — The v2 self-eval Phase 2 prose said "The header now contains: logo + tagline (left), 5 tabs with icons + labels (center), Charles button + Wallet pill + Logout button (right). That's a lot of content for one horizontal bar." C-6 and B-34 treat this as a CSS sizing problem (shrink fonts, add overflow handling). But the v2 self-eval raised it as a *density* problem — should the tagline be removed from the header on desktop? Should the Charles button only appear for admin principals? Should the logo be smaller? No item in the v3 plan considers reducing the number of elements in the header. It only proposes making the existing elements fit better.

44. **Design Philosophy doc prescribes a layout that no longer exists** — The Design Philosophy doc's Layout Rules section still says "Left rail navigation (200px, always labeled — do NOT collapse to icon-only)" and "Trollbox panel: fixed right, ~300px, always visible." The v2 work deleted the sidebar. The trollbox is explicitly deferred. Anyone reading the Design Philosophy doc as a handoff guide (which is its stated purpose: "For any agent continuing UX/frontend work on this project") would try to build a sidebar and a trollbox panel because the document tells them to. The doc needs to be updated to reflect the current header-tab navigation and the trollbox deferral.

45. **MobileSheet reusable component doesn't include drag-to-dismiss** — Phase 1.2 of the v3 plan fixes the drag handle in WalletDropdown by adding touch event handlers. Phase 16.5 creates a reusable `MobileSheet` component for all other dialogs, reusing the `mc-bottom-sheet` CSS and including a `mc-drag-handle` div — but the code sample has no touch event handlers on the handle. The fix from Phase 1.2 is scoped to WalletDropdown specifically, not to the reusable component. This means the three new bottom sheets (withdrawal, reinvest, shenanigan cast) would all ship with decorative drag handles — the exact bug that C-2 identified.

46. **countUp `resetToken` pattern not established for future usages** — Phase 5.1 adds a `resetToken` to `useCountUp` so the Profit Center numbers re-animate on tab switch. Phase 13.1 applies `useCountUp` to the ROI calculator in GamePlans but doesn't use the `resetToken` pattern. Any future `useCountUp` usage that doesn't include the reset pattern will have the same "first-load only" bug that A-10 identified. The fix should be built into the hook itself (auto-reset on visibility change) rather than requiring each consumer to remember to pass a token.

Some of these were reasonable to scope out (network visualization is a big feature, animated background is cosmetic, trollbox is a major feature). But several — especially the onboarding tour, typography audit, duplicate refresh button check, docs page, PP enrichment, `prefers-reduced-motion`, and Charles personality — are exactly the kind of detail work that keeps getting deferred. The v2 plan's thesis was "fix everything the v1 missed." It didn't. It fixed the *structural* things the v1 missed and quietly dropped the polish, two entire original phases, three items lost between the original report and the v1 task list, nine items that fell through the cracks between the original report and the v1 self-evaluation, seven items that survived every previous audit round, seven items from within the v2 plan's own specifications that were never implemented to spec, and seven more found by auditing the Design Philosophy's own rules and reading the v2 self-evaluation's prose more carefully than its author did.
