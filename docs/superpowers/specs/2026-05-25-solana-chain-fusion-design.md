...
4. **Cover charge SOL destination — resolved.** Mirrors the existing ICP cover charge flow:
   ...
   - `solTreasuryAddress : ?Text` config field on `ponzi_math_sol`, admin-tunable. **Set to `5EVdR6qcPuDqJb6W69fmcvTJjbEUGZqQtefEB8sK8QQ2` at deploy** — the operator's personal Phantom address. Verify the value matches this string exactly before the first `payManagementSol` sweep on mainnet; sending SOL to a wrong address is irreversible.
